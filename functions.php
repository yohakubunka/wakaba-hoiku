...
{
  add_menu_page('トップページ設定', 'トップページ設定', 'manage_options', 'post.php?post=18&action=edit', '', 'dashicons-admin-generic', 4);
}
...
//アーカイブのディスクリプション
function addArchiveDescription()
{
  $post_types = [
    'post' => 'お知らせ'
  ];
  ...
  foreach ($post_types as $key => $val) {
    add_settings_field($key . '_description', $val . 'のディスクリプション', 'exArchiveDescription', 'writing', 'default', [$key]);
    register_setting('writing', $key . '_description');
  }
}
add_filter('admin_init', 'addArchiveDescription');

function exArchiveDescription($args)
{
  $key = $args[0] . '_description';
  $val = get_option($key);
?>
  <textarea name="<?= $key ?>" id="<?= $key ?>" rows="8" cols="80" class="regular-text"><?= esc_html($val); ?></textarea>
<?php
}
//管理画面お知らせ表示
// function post_menu_label()
// {
//   global $menu, $submenu;
//   $name = 'お知らせ';
//   $menu[5][0] = $name;
...
add_action('init', 'post_object');
// add_action('admin_menu', 'post_menu_label');
//パンくずリストに出力される不要な部分を非表示
...
function user_remove_menus()
{
  ...
  if ($current_user->ID == "2") //４は設定メニューを非表示
  {
